...
import { Home, Compass, PlusSquare, Bell, User as UserIcon, LogOut, Sun, Moon, ShieldCheck, Palette, Check, Sparkles } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab, onCreateClick }) => {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row font-light bg-white dark:bg-slate-950 transition-colors duration-500">
      ...
      <aside className="hidden md:flex flex-col w-72 h-screen sticky top-0 p-8 z-40 border-r border-slate-100 dark:border-white/5 bg-white/70 dark:bg-slate-900/40 backdrop-blur-xl">
            ...
            return (
              ...
              >
                <div className={`transition-colors duration-300 relative z-10 ${isActive ? 'text-white' : 'text-slate-400 group-hover:text-opacity-100'}`} style={!isActive ? { color: 'inherit' } : {}}>
                  {/* Fix: Casting to React.ReactElement<any> to allow the spread of lucide-react specific props like size and strokeWidth */}
        ...
        <div className="mt-auto space-y-6">
          ...
          <div className="p-6 bg-slate-100/50 dark:bg-black/40 rounded-3xl border border-slate-200 dark:border-white/5 space-y-5 shadow-sm">
            ...
            <div className="text-[10px] font-bold text-center opacity-60 dark:opacity-40 uppercase tracking-widest" style={{ color: accent.hex }}>
              {accent.name}
            </div>
          </div>

          <div className="p-4 bg-white dark:bg-slate-900/60 rounded-2xl flex items-center gap-4 border border-slate-200 dark:border-white/5 group cursor-pointer hover:border-slate-300 transition-all shadow-sm">
            <div className="relative">
              {MOCK_USER.avatar ? (
                <img src={MOCK_USER.avatar} alt="Profile" className="w-10 h-10 rounded-full object-cover border-2 border-slate-100 dark:border-slate-800" />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold truncate text-slate-900 dark:text-white uppercase tracking-wider">{MOCK_USER.name}</p>
              <p className="text-[9px] text-slate-500 dark:text-slate-400 truncate font-black tracking-widest uppercase">Elite Verified</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      {/* Bottom Nav - Mobile (Fixed to bottom, no floating glitch) */}
        ...
        <div className="flex justify-around items-center h-16 px-4">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <div 
                  className={`transition-all duration-300 ${isActive ? 'text-white p-2 rounded-lg' : 'p-2'}`}
                  style={isActive ? { backgroundColor: accent.hex } : {}}
                >
                  {/* Fix: Casting to React.ReactElement<any> to allow the spread of lucide-react specific props like size and strokeWidth */}
                ...
                {isActive && (
                  <div className="absolute -bottom-1 w-1 h-1 rounded-full bg-current mt-1" style={{ backgroundColor: accent.hex }}></div>
                )}
              </button>
            );
          })}
